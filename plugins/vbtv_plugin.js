...
function getManifest() {
    return JSON.stringify({
        ...
        "name": "VeboTV",
        "version": "1.0.0",
        "baseUrl": "https://vbtv.pro",
        "iconUrl": "https://vbtv.pro/vebo-icon.png",
        "isEnabled": true,
        "type": "VIDEO",
...
function getHomeSections() {
    return JSON.stringify([
        { slug: 'live', title: '🔴 Bóng Đá Live', type: 'Grid', path: '' }
    ]);
}
...
function parseListResponse(apiResponseJson) {
    try {
        ...
        groups.forEach(function (group) {
            // Bỏ qua các group không phải trận đấu (tournaments, related, replay, highlight...)
            if (group.groups) return; // nested groups = tournaments/related section
...
function parseMovieDetail(apiResponseJson, apiUrl) {
    try {
        ...
        var description = "Trực tiếp bóng đá - VeboTV";
